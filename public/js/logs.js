...
new _infinite_1.Infinite('/api/list-logs', function (log) {
  var $log = document.createElement('div');
  if (log.type == util_1.LogType.info) {
    $log.className = 'log info-log';
    $log.innerHTML = ICONS.infoCircle;
  }
  if (log.type == util_1.LogType.warn) {
    $log.className = 'log warn-log';
    $log.innerHTML = ICONS.alertTriangle;
  }
  if (log.type == util_1.LogType.error) {
    $log.className = 'log error-log';
    $log.innerHTML = ICONS.alertHexagon;
  }
  var $source = document.createElement('div');
  $source.className = 'source';
  $source.innerText = "[".concat((0, util_1.time)(new Date(log.date)), "] ").concat(log.source, ": ");
